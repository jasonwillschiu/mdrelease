...
- `mdrelease version` must print `<latest-changelog-version>` only (plain semver, no repo prefix, no `v` prefix).
- Root `--help` output must include the installed mdrelease version string.
